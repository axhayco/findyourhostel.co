      ...
      >
        <Heart
          className={`h-6 w-6 drop-shadow-md ${
            isFav ? "fill-primary text-primary" : "fill-foreground/30 text-card stroke-[1.5]"
          }`}
        />
      </button>
    </div>
        ...
        <div className="flex gap-1 overflow-x-auto px-4 py-3 scrollbar-hide">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`flex flex-shrink-0 flex-col items-center gap-1 px-4 py-1.5 transition-all ${
                category === cat.id
